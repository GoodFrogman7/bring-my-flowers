...
Business mode ships a **local dashboard** at `http://localhost:8787` (same machine
as the bot). It is designed for a non-technical owner: large type, plain tabs,
one-click sheet download, a **Review** queue for messages the parser refused to
guess on, and an **Ask the bot** chat box.
...
- Personal chats are ignored.
- Owner DMs receive **only the delivery sheet** — never customer chat.
